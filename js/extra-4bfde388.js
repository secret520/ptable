...
function c(e, t, n = e.target.href) {
  function o(e) {
    ...
    t &&
      ((t.style.colorScheme = document.documentElement.style.colorScheme),
      (t.style.color = window
        .getComputedStyle(document.documentElement)
        .getPropertyValue("color")));
  }
  if (
    screen.width * screen.height < 614400 ||
    !navigator.onLine ||
    e.altKey ||
    e.ctrlKey ||
    e.shiftKey ||
  ...
  const c = function () {
    ...
    return {
      top: e.top,
      left: e.left,
      bottom: document.documentElement.clientHeight - e.bottom,
      right: document.documentElement.clientWidth - e.right,
    };
  }.call(t);
  for (const e in c) a.style[e] = c[e] + "px";
  a.classList.add("Clone", ...t.classList);
  const s = document.createElement("button");
  s.addEventListener("click", i, { capture: !0 }),
    a.appendChild(s),
  ...
  const l = (n = n.split("#"))[0]
      .replace(/ /g, "_")
      .replace(/^.*\/\/([a-z-]+).wikipedia.org\/wiki\//, "/wiki/$1/A/"),
    d = n[1] ? `#${n[1]}` : "";
  (r.src = `${l}${d}`),
    (r.onload = function (e) {
      o({ target: this });
    ...
    a.addEventListener(
      ...
      function (e) {
        "top" === e.propertyName &&
          (this.classList.add("ExpandDone"), o({ target: r }));
      },
      !0
    );
...
o("CloseUp")
  ...
  .addEventListener("click", (e) => {
    let t;
    o("CloseUp").classList.contains("Tope") &&
      (t = e.target.href.replace("//en.", "//isotopes.")),
      c(e, o("CloseUp"), t);
  }),
  o("Series")
    .querySelectorAll("a")
    .forEach((e) =>
      e.addEventListener("click", (t) => c(t, e.closest("dt, dd")))
    ),
  o("StateOfMatter")
    .querySelectorAll("a")
  ...
  o("DecayModes")
    ...
    .forEach((e) => e.addEventListener("click", (t) => c(t, e.closest("div")))),
  document
    .querySelector('[value="name"] + li')
    .addEventListener("click", (e) => {
      "A" === e.target.nodeName && c(e, o("CloseUp"));
    }),
  document
    .querySelector('[value="isoname"] + li')
    .addEventListener("click", (e) => {
  ...
  o("Ptable")
    ...
    .forEach((e) =>
      e.addEventListener("click", (t) => c(t, e.closest(".Group")))
    ),
  ...
  o("Ptable").addEventListener("activateElement", async (t) => {
    ...
    if (
      t.target.classList.contains("Tope") &&
      document.querySelector(".Isotope .isoname")
    ) {
      const n = `https://isotopes.wikipedia.org/wiki/${await e.value(
        t.target.dataset.atomic,
        t.target.dataset.neutrons,
        "isowiki"
      )}`;
      c(t, t.target, n);
    }
...
(s("YearRange").oninput = s("Year").oninput = u),
  s("YearAdd").addEventListener("pointerdown", d),
  s("YearSub").addEventListener("pointerdown", d),
  (document.querySelector("#SliderYear > label").onclick = m),
...
function p() {
  return "requestIdleCallback" in window
    ...
    : new Promise((e) => setTimeout(e, 1));
}
async function g(e) {
  function t(e) {
    this.classList.toggle("NoSelect", !e),
      this.querySelectorAll("a, input, select, [tabindex]").forEach(
        (t) => (t.tabIndex = e ? 0 : -1)
      ),
      this.querySelectorAll("select").forEach(
        (e) => (e.disabled = !Boolean(e.options.length - 1))
      );
  }
...
!(async function () {
  await p(),
    "IntersectionObserver" in window &&
      (f(h("KeyRegion"), g), f(h("DataRegion"), g));
})();
...
function L() {
  document.removeEventListener("keydown", w),
    y && y.disconnect(),
    document.querySelector("aside").removeEventListener("changeProperty", S),
    document.body.classList.remove("SearchActive"),
    (v("SearchButton").checked = !1),
    (E.value = ""),
    k();
}
function b(e) {
...
async function k() {
  ...
  ["name", "series"].includes(a.value) &&
    (a = document.querySelector('input[value="weight"]'));
  const c = await t(a, "search");
  let i, s, r;
  if (e.atomic_map.has(Number(o))) i = e.atomic_map.get(Number(o));
  else {
    let e = [...c].filter(
      ([e, t]) => e.children[0].textContent.toLowerCase() === o
    );
    e.length && (i = e[0][0]);
  }
  "-" === o.charAt(0)
    ? ((r = "~"), (s = Number(o)))
    : [">", "<", "=", "~"].includes(o.charAt(0))
    ? ((r = o.charAt(0)), (s = Number(o.substring(1))))
    : o.substring(1).includes("-") || o.includes("..")
    ? ((r = "-"), (s = o.split(/-|\.\./)))
    : isNaN(o)
    ? ((r = "S"), (s = o.toLowerCase()))
    : ((r = "~"), (s = Number(o))),
    ["oxidation"].includes(a.value) && (r = "S");
  let l = [];
  if ("=" === r) l = [...c].filter(([e, t]) => t == s);
  else if ("<" === r) l = [...c].filter(([e, t]) => Number(t) < s);
  else if (">" === r) l = [...c].filter(([e, t]) => Number(t) > s);
  else if ("-" === r && 2 === s.length)
    l = [...c].filter(
      ([e, t]) =>
        t >= Number(s[0] || Number.MIN_VALUE) &&
        t <= Number(s[1] || Number.MAX_VALUE)
    );
  else if ("~" === r) {
    let e = [...c.values()]
      .filter((e) => null != e)
      .reduce((e, t) => Math.min(Math.abs(t - s), e), Number.MAX_VALUE);
    l = [...c].filter(([t, n]) => Math.abs(n - s) <= e);
  } else
    "S" === r &&
      (l = [...c].filter(([e, t]) => String(t).toLowerCase().includes(s)));
  l.length ||
    (l = [...c].filter(
      ([e, t]) => e.children[1].textContent.toLowerCase() === s
    )),
    l.length ||
      (l = [...c].filter(([e, t]) =>
        e.children[2].textContent.toLowerCase().includes(s)
      )),
    (l = l.map((e) => e[0])),
    l.length
      ? i && (l[0] !== i || l.length > 1)
        ? ("atomic" === n && (l = [i]),
          v("SearchType").classList.add("Ambiguous"))
        : v("SearchType").classList.remove("Ambiguous")
      : (i && l.push(i), v("SearchType").classList.remove("Ambiguous")),
    (function (t) {
      document
        .querySelectorAll("ol > li")
        .forEach((e) => e.classList.toggle("Dim", !t.includes(e))),
        1 === t.length &&
          (t[0].dispatchEvent(e.hoverElement), t[0].scrollIntoView(!1));
    })(l);
}
...
function A(t) {
  const n = document.activeElement.dataset.atomic,
    ...
    a = {
      ArrowDown: (e) => q(e, 1),
      ArrowUp: (e) => q(e, -1),
      ArrowLeft: (e) => Math.max(1, --e),
      ArrowRight: (e) => Math.min(++e, o),
    };
  if (!n || !a[t.key]) return;
  const c = Number(n),
    i = a[t.key](c);
...
function q(t, o) {
  const a = ((e) => document.getElementById(e))("Wide").checked
      ? e.table_map.wide
      : e.table_map.traditional,
    c = n(t, a);
  let i;
  return (
    (c[1] += o),
    7 === c[1] && (c[1] += o),
    c[1] >= 0 && c[1] < a.length && (i = a[c[1]][c[0]]),
    i || t
  );
}
...
function C() {
  N("Ptable").classList.remove(...T),
    N("Ptable").classList.add(this.classList.value);
}
N("Blocks")
  .querySelectorAll("dt")
...
[...document.querySelectorAll("#StateOfMatter dt")].forEach((e) => {
  M.push(e.className),
    e.addEventListener("pointerenter", () => x(e.className)),
    e.nextElementSibling.addEventListener("focusin", () => x(e.className)),
    e.nextElementSibling.addEventListener("focusout", () => B(e.className));
}),
...
function W() {
  D("Ptable").classList.remove(..._),
    this.dataset.classes
      ? D("Ptable").classList.add(...this.dataset.classes.split(" "))
      : D("Ptable").classList.add(...this.classList);
}
[...D("Series").children].forEach((e) => {
  e.addEventListener("pointerenter", W),
...
function O(t) {
  ...
  o && o.dispatchEvent(e.syncThemes),
    ($("DarkButton").checked = this.checked),
    ($("DarkButtonFooter").checked = this.checked),
    t.isTrusted &&
      localStorage.setItem("Theme", this.checked ? "Dark" : "Light");
  const a = document.querySelector(e.activeProperty);
  a && a.dispatchEvent(e.recolorTable),
    document.documentElement.offsetHeight,
    document.documentElement.classList.remove("NoTransitions");
}
$("DarkButton").addEventListener("change", O),
  $("DarkButtonFooter").addEventListener("change", O),
  ($("DarkButton").checked = $("DarkButtonFooter").checked =
    document.documentElement.classList.contains("Dark"));
const F = (e) => document.getElementById(e);
let H;
function U(e) {
  (e.key && "Enter" !== e.key) ||
    ((F("Wide").checked = !0), j.call(F("Wide"), { isTrusted: !0 }));
}
function j(e) {
  (F("Wide").checked = this.checked),
    (F("WideFooter").checked = this.checked),
    clearTimeout(H);
...
async function z(e) {
  ...
  t >= 0 &&
    ...
    (document.documentElement.dataset.wide = t),
    document.documentElement.style.setProperty(
      "--groups",
      Math.min(32, t + 18)
    ),
    (H = setTimeout(() => z(e), 120)));
}
!(function () {
  F("Wide").addEventListener("change", j),
    F("WideFooter").addEventListener("change", j),
    F("Wide").dispatchEvent(new Event("change")),
    F("VerticalInner").addEventListener("click", U),
    document
      .querySelectorAll(".Label57to71, .Label89to103")
      .forEach((e) => e.addEventListener("keypress", U));
  const t = Array(15)
    .fill()
    .map((e, t) => t + 1);
  for (let n = 0; n < 15; n++)
    (e.atomic_map.get(103 - n).dataset.wide = t.slice(n).join(" ")),
      (e.atomic_map.get(71 - n).dataset.wide = t.slice(n).join(" "));
})();
function Y() {
...
function X(e) {
  e.matches &&
    V("SidebarToggle").checked &&
    (document.documentElement.classList.toggle("Sidebar", !1),
    (V("TopbarToggle").checked = !0));
}
function J() {
  K.removeEventListener("transitionend", Q),
    K.removeEventListener("transitionend", Z),
    document.documentElement.classList.remove("Collapse", "SmoothResize"),
    document.documentElement.offsetHeight,
    document.documentElement.classList.contains("Reset")
      ? G()
      : (document.documentElement.classList.add("SmoothResize", "Collapse"),
        K.addEventListener("transitionend", Q));
}
function G() {
  document.documentElement.classList.toggle(
    ...
    localStorage.setItem(
      "Layout",
      V("SidebarToggle").checked ? "Sidebar" : "Topbar"
    );
}
function Q(e) {
  e.target === this &&
    e.propertyName.startsWith("flex") &&
    (this.removeEventListener("transitionend", Q),
    G(),
    document.documentElement.classList.remove("Collapse"),
    K.addEventListener("transitionend", Z));
}
function Z(e) {
  e.target === this &&
    e.propertyName.startsWith("flex") &&
    (this.removeEventListener("transitionend", Z),
    document.documentElement.classList.remove("SmoothResize"));
}
!(function () {
  (V("SidebarToggle").checked =
    document.documentElement.classList.contains("Sidebar")),
    V("SidebarToggle").addEventListener("change", J),
    V("TopbarToggle").addEventListener("change", J),
    document
      .querySelector("#ResetTab + label")
...
function ne() {
  ...
  (e.src =
    "https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-1899956715581914"),
  ...
  t &&
    t.addEventListener("click", function (e) {
      ...
      t &&
        ((t.dataset.adsbygoogleStatus = ""),
        (t.style.display = e.target.parentNode.style.display = "none"),
        (t.parentNode.style.minHeight = 0));
    });
}
function oe(e) {
  const t = te("Notice");
  switch (Number(e)) {
    case 1500:
      setTimeout(() => location.reload(!0), 60 * 120 * 1e3),
...
!(function () {
  ...
  const t =
    ((n = "PatreonSubscription"),
    new Map(document.cookie.split(";").map((e) => e.trim().split("="))).get(n));
  var n;
  t
    ? oe(t)
    : te("Notice") &&
...
function ce() {
  ae("Ptable").classList.remove("Shine", "Delay"),
    document.removeEventListener("pointerdown", ce);
}
function ie(e) {
  const t = [
    `/?lang=${e}`,
    ...
    "/JSON/compounds.json",
    ...[...document.querySelectorAll('footer [href$=".svg"]')].map(
  ...
  return {
    ...
    enableAutoPageviews: () => {
      ...
      return (
        ...
        n(),
        function () {
          o && ((history.pushState = o), removeEventListener("popstate", t)),
      ...
      function c(e) {
        e instanceof HTMLAnchorElement
          ? e.host !== location.host &&
            (e.addEventListener("click", o), a.add(e))
          : "querySelectorAll" in e && e.querySelectorAll("a").forEach(c);
      }
      function i(e) {
        e instanceof HTMLAnchorElement
          ? (e.removeEventListener("click", o), a.delete(e))
          : "querySelectorAll" in e && e.querySelectorAll("a").forEach(i);
      }
      const s = new MutationObserver((e) => {
        e.forEach((e) => {
          "attributes" === e.type
            ? (i(e.target), c(e.target))
            : "childList" === e.type &&
              (e.addedNodes.forEach(c), e.removedNodes.forEach(i));
        });
      });
      return (
...
async function le() {
  await ("requestIdleCallback" in window
    ? new Promise(requestIdleCallback)
    : new Promise((e) => setTimeout(e, 1))),
    (function () {
      const e = document.documentElement.dataset.iso,
        t = document.location.hostname.includes(".dev")
          ? "ptable.dev"
          : "ptable.com",
...
((e) => document.getElementById(e))("Share").addEventListener("click", se),
  "complete" === document.readyState
    ? le()
...
async function pe(e) {
  ...
  let n;
  await de(),
    (n = e.map((e) => t(e.children[2]))),
    await ue(),
    e.forEach((e, t) => me.call(e.children[2], n[t])),
    await de(),
    (n = e.map((e) => t(e.children[2])));
